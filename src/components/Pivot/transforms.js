...
const transforms = function(){
  ...
  function mapper(where, datapointCol, animationCol, data){
    ...
    const dates = metadata.getDatesAndDateStrings();
    ...
    return applyWhereClause(data, where, datapointCol).map(function(datum){
      ...
      //
      try {
        const bin = datapoint.getBinningFn(datapointCol)(datum, datapointCol);
  ...
  // This just returns counts for each variable.
  //
  ...
  // Sum up every numeric variable, except for those marked noSummary
  // and singletons.
  //
  function getNumericSummaries(data){
    const nonSummaryNumerics = metadata.getNumerics()
      .filter(i => !metadata.isTrue(i, 'noSummary') && 
                   !metadata.hasAttribute(i, 'singleton'));
    const numericMap = nonSummaryNumerics.reduce((i, j) => {
      return {...i, ...{[metadata.getAlias(j)]: true}};
    }, {});
  ...
  // Return values for every singleton data, or empty object if there is no data
  //
  function getSingletons(data){
    const first = data.length > 0 ? data[0] : null;
    if (!first) {
      return {};
    }
    const res = metadata.getSingletonNumerics().reduce((i, j) => {
      const alias = metadata.getAlias(j);
      return (first.hasOwnProperty(alias)) ? {...i, [alias]: first[alias]} : i;
    }, {});
    return res;
  }
