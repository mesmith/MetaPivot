...
const getNewDatasetState = (dataset, filter, datapointCol, loadTable) => 
  ...
  const initControlState = controls.getInitControlState(categoricalValues, 
      datapointCol, 'line');
  const facet = getFacetObject(pivotedData, filter, datapointCol, categoricalValues);
...
//
const getFacetObject = (pivotedData, filter, datapointCol, categoricalValues) => {
  ...
  const thisDatapoint = (originalDatapointCol && datapointCol === dfltDatapointCol)
    ? originalDatapointCol
    : datapointCol === null ? dfltDatapointCol: datapointCol;
  const facetList = facets.getReactFacets(pivotedData,
      filter, thisDatapoint, categoricalValues);
