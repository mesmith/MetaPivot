...
// import { customer as cust } from './env.js';
...
const metadata = function(){
  ...
  //   See also dataread withAverages().
  // multiline (string)
  //   A list of others column thats will be plotted along with the 
  //   current column in a line chart.
  // noAxis (boolean)
  ...
  // noWhatIf (boolean)
  //   If true, do not include this column in the what-if variable chooser.
  // obscure (boolean)
  //   If true, do not show the values in tooltips
  // onlyWithDatapoint (vector of columns)
  //   If specified, then this column is only calculated if the current
  //   datapoint is one of the columns in the vector of column names.
  //   Only has effect for columns with calculated === true.
  // output
  //   The name of the column that is generated by this column.
  //   Currently used to convert DateString columns into Date columns
  //   (which hold the date in milliseconds).
  // searchable (boolean)
  //   If true, the user can do a faceted search on the values of this
  //   column
  // singleton (string)
  //   Specifies that this is a singleton field; that is, a field that
  //   has the same value for either a single datapoint, or for the entire
  //   dataset.
  //   Its value is the name of the datapoint column for which it is singleton.
  //   If its value is '', it means that this column is constant across the
  //   dataset.
  // summary (boolean)
  ...
  //     'Date' indicates a date in milliseconds.  Used to generate a time series axis.
  ...
  const airForceMetadata = {
    dataset: {
      ...
      name: 'test1000', alias: 'Air Force Recruitment',
  ...
  const ticketSubjectMetadata = {
    ...
    'Assigned Account': {
      alias: 'Analyst', type: 'Categorical', datapoint: true, tooltip: false,
      summary: true
    },
    'Incident S/A': {
  ...
  const callMetadata = {
    ...
    dataset: {
      ...
      alias: 'Calls with Tickets', 
      graphtype: 'line',
      disabled: [ 'animate' ],

      markers: {
        datapoint: 'month',  // Valid for this datapoint...
        data: [
          { name: 'Covid Travel Ban', date: '1/31/2020' },
          { name: 'Covid Peak 1', date: '7/22/2020' },
          { name: 'Covid Peak 2', date: '1/10/2021' },
          { name: 'Covid Peak 3', date: '9/13/2021' },
          { name: 'Pop Flow Deployed',   date: '4/7/2020' },
          { name: 'Pop Flow Suspended',  date: '8/1/2021' },
          { name: 'Callback Deployed',  date: '1/18/2022' },
          { name: 'Pop Flow Resumed',    date: '9/20/2022' },
        ],
      },
      ...
      //
      // 
      // abandonedCallsRegular_1Q2019: 36177,
      // abandonedCallsNW_1Q2019: 4034,

      // abandonedCallsRegular: 24303,  // From $ npm run abandonment-2021
      // abandonedCallsNW: 4126,        // From $ npm run abandonment-2021

      // Ratio of abandoned calls that represent balkers.  Obtained by analyzing
      // the hazard rate of the abandoned calls: an initial spike of abandonments
      // in the abandonent histogram is likely to represent callers that were not
      // seriously looking for service.  We take the # of values that appear to
      // be balkers and divide by the total # of values.
      //
      // For the newer datasets, it appears that the constant hazard rate is around 4;
      // the # of early abandonments is around 6000 out of total of 30000, closer to .20.
      //
      balkPercent_1Q2019: .15,
      balkPercent: .18,
    },
    'ID': { // primary key
      alias: 'ID', type: 'String', tooltip: true, tooltipIndex: 0
    },
    '# Records': { // sum of primary key (calls)
      alias: '# Handled Calls', type: 'Number', tooltip: true, tooltipIndex: 0,
      summary: true,
      defaultYValue: 'self',
    },

    // Currently we treat these date constants as categorical.  This will cause
    // the name to be pivoted with the value in the name.  Ugly.
    // FIXME: Handle this cleanly in the mongo pipeline.
    //
    'DatasetStartTime': {
      type: 'Categorical',
      dataRange: 'begin',
    ...
    month: {
      alias: 'Month',
      defaultDatapoint: true,
      type: 'DateString', output: 'month_ms',
      subtype: 'Series',  // indicates that this will be used to plot time series
      datapoint: true,
      noAxis: true,
      searchable: true,
      graphtype: 'line',  // causes line chart to be drawn by default
      noWhatIf: true,
    },
    'month_ms': {
      type: 'Date',
      alias: 'Month ',   // NOTE deconflicting space in alias
      defaultXValue: 'self',
      onlyWithDatapoint: [ 'month' ],
      noWhatIf: true,
      noAggregate: true,  // not needed when getting data from DB
    },
    ...
    'Target Name': {
      alias: 'Target Analyst',
      type: 'Categorical',
      noAggregate: true,   // Use analyst below to aggregate; avoid data duplications.
      noAxis: true,
      noWhatIf: true,
      summary: true
    },

    // Artificial columns here (see ETL correlators, e.g. TAC-join-*):
    // Used in e.g. modelchart.js, models.js metadata.
    //
    analyst: {
      alias: 'Analyst',
    ...
    xUtilization: {
      ...
      calculated: {
        idx: 1,
        transform: models.getUtilization,
        fields: ['ticketSummary_daysWorkPerCall', 'ticketSummary_secsPerCall'],
      },
      tooltip: true, tooltipIndex: 101,
    },
    ...
    ticketSummary_minutesPerCallAvg: {  // calculated in dataread withAverages()
    ...
    talkMinutesAvg: {  // this just includes time the analyst and traveler talk
                       // calculated in dataread withAverages()
...
// Put these two back later
    ...
    'Median Edit Time': {
      alias: 'Phone Ticket Work Time (Median), Seconds',
      noAxis: true,
      type: 'Number',
      tooltip: true
    },
    ...
    // Abandonment data, merged from the abandonment dataset
    //
    abandoned_total: {
      alias: 'Abandoned (total)',
      type: 'Number',
      singleton: '',  // value is constant everywhere
      noAxis: true,
    },
    abandoned_regular_total: {
      alias: 'Abandoned regular shift (total)',
      type: 'Number',
      singleton: '',  // value is constant everywhere
      noAxis: true,
    },
    abandoned_nw_total: {
      alias: 'Abandoned N/W shift (total)',
      type: 'Number',
      singleton: '',  // value is constant everywhere
      noAxis: true,
    },
    ...
    abandoned_month: {
      alias: '# Abandoned Calls', 
      type: 'Number',
      singleton: 'month', // value is constant for each 'month' datapoint
      onlyWithDatapoint: [ 'month' ],
      multiline: [
        { col: 'abandoned_regular_month', color: '#777' },
        { col: 'abandoned_nw_month', color: '#555' },
      ],
    },
    abandoned_regular_month: {
      alias: '# Abandoned Calls, regular shift',
      type: 'Number',
      singleton: 'month', // value is constant for each 'month' datapoint
      onlyWithDatapoint: [ 'month' ],
    },
    abandoned_nw_month: {
      alias: '# Abandoned Calls, N/W shift',
      type: 'Number',
      singleton: 'month', // value is constant for each 'month' datapoint
      onlyWithDatapoint: [ 'month' ],
    },

    // Categorical summary calculated fields below here.
    //
    // These fields are generated from fields above, so e.g. 'Target Name' will
    // create a new field called '# Analyst'.  In turn, this means that we
    // cannot change 'alias' here unless we change the alias for Target Name.
    //
    // See also summary.js, getColumnsWithAttrTrue('summary'), where the variable name
    // is generated.
    //
    nAnalyst: {
      alias: '# Analyst',  // Alias must be derived from another 'summary' variable!!
      type: 'Number',
      tooltip: true, tooltipIndex: 100, tooltipSeparator: true,
      noSummary: true,  // Do not count this field--it interferes with TargetName above
      noAggregate: true,  // not needed when getting data from DB
      multiline: [
        { col: 'xTotalLowerBound', color: 'yellow' },
        { col: 'xTotalUpperBound', color: 'green' },
      ],
    },
    ...
    estimate_month: {
      ...
      onlyWithDatapoint: [ 'month' ],
      multiline: [
        { col: 'actual_month', color: '#777' },
        { col: 'xEstimateDiff', color: 'purple' },
      ],
    },
    ...
    actual_month: {
      ...
      onlyWithDatapoint: [ 'month' ],
      multiline: [
        { col: 'estimate_month', color: '#777' },
        { col: 'xEstimateDiff', color: 'purple' },
      ],
    },

    // Directly calculated fields below here.
    //
    xEstimateDiff: {
      alias: 'Estimate vs Actual, $1000',
      type: 'Number',
      singleton: 'month',
      onlyWithDatapoint: [ 'month' ],
      calculated: {
        idx: 1,
        transform: utils.getEstimateDiff,
        fields: ['estimate_month', 'actual_month'],
      },
      tooltip: true, tooltipIndex: 101,
    },

    xTrafficIntensity: {
      alias: 'Traffic Intensity',
      type: 'Number',
      onlyWithDatapoint: [ 'month' ],
      calculated: {
        idx: 1,
        transform: models.getTrafficIntensity,
        fields: [],
      },
      tooltip: true, tooltipIndex: 101,
    },
    xArrivalRate: {
      alias: 'Arrival Rate (Calls/Min)',
      type: 'Number',
      onlyWithDatapoint: [ 'month' ],
      calculated: {
        idx: 1,
        transform: models.getArrivalRate,
        fields: [],
      },
      tooltip: true, tooltipIndex: 101,
    },
    xDailyHandlingTimePerAnalyst: {
      alias: 'Daily Handling Time per Analyst (Hours)',
      type: 'Number',
      onlyWithDatapoint: [ 'month' ],
      calculated: {
        idx: 1,
        transform: models.getDailyHandlingTimePerAnalyst,
        fields: [],
      },
      tooltip: true, tooltipIndex: 101,
    },
    xAverageHandleTime: {
      alias: 'Average Handling Time per Call (Mins)',
      type: 'Number',
      onlyWithDatapoint: [ 'month' ],
      calculated: {
        idx: 1,
        transform: models.getAverageHandleTimePerCall,
        fields: [],
      },
      tooltip: true, tooltipIndex: 101,
    },
    xConcurrentWorkingAnalysts: {
      alias: '# Concurrent Working Analysts',
      type: 'Number',
      onlyWithDatapoint: [ 'month' ],
      calculated: {
        idx: 1,
        transform: models.getConcurrentWorkingAnalysts,
        fields: [],
      },
      tooltip: true, tooltipIndex: 101,
      multiline: [
        { col: 'xConcurrentLowerBound', color: 'yellow' },
        { col: 'xConcurrentUpperBound', color: 'green' }
      ],
    },
    xAnalystAggregateUtilization: {
      alias: 'Analyst Aggregate Utilization',
      type: 'Number',
      onlyWithDatapoint: [ 'month' ],
      calculated: {
        idx: 1,
        transform: models.getAnalystAggregateUtilization,
        fields: [],
      },
      tooltip: true, tooltipIndex: 101,
    },
    xAbandonmentRate: {
      alias: 'Abandonment Rate (%)',
      type: 'Number',
      onlyWithDatapoint: [ 'month' ],
      calculated: {
        idx: 1,
        transform: models.getAbandonmentRate,
        fields: [],
      },
      tooltip: true, tooltipIndex: 101,
    },
    xPatience: {
      alias: 'Patience until Abandonment (mins)',
      type: 'Number',
      onlyWithDatapoint: [ 'month' ],
      calculated: {
        idx: 1,
        transform: models.getPatience,
        fields: [],
      },
      tooltip: true, tooltipIndex: 101,
    },
    xAnalystsAdjusted: {
      alias: '# Analysts Adjusted for Days Worked',
      type: 'Number',
      onlyWithDatapoint: [ 'month' ],
      calculated: {
        idx: 1,
        transform: models.getAnalystsAdjustedForDaysWorked,
        fields: [],
      },
      tooltip: true, tooltipIndex: 101,
      multiline: [
        { col: 'nAnalyst', color: '#555' },  // can see total # of analysts on this chart
      ],
    },

    xConcurrentUpperBound: {
      alias: '# Concurrent Analysts Needed, Upper Bound',
      type: 'Number',
      onlyWithDatapoint: [ 'month' ],
      calculated: {
        idx: 1,
        transform: models.getConcurrentAnalystsNeededUpperBound,
        fields: [],
      },
      tooltip: true, tooltipIndex: 200, tooltipSeparator: true,
    },
    xTotalUpperBound: {
      alias: '# Total Analysts Needed, Upper Bound',
      type: 'Number',
      onlyWithDatapoint: [ 'month' ],
      calculated: {
        idx: 1,
        transform: models.getTotalAnalystsNeededUpperBound,
        fields: [],
      },
      tooltip: true, tooltipIndex: 201,
    },

    xConcurrentLowerBound: {
      alias: '# Concurrent Analysts Needed, Lower Bound',
      type: 'Number',
      onlyWithDatapoint: [ 'month' ],
      calculated: {
        idx: 1,
        transform: models.getConcurrentAnalystsNeededLowerBound,
        fields: [],
      },
      tooltip: true, tooltipIndex: 200, tooltipSeparator: true,
    },
    xTotalLowerBound: {
      alias: '# Total Analysts Needed, Lower Bound',
      type: 'Number',
      onlyWithDatapoint: [ 'month' ],
      calculated: {
        idx: 1,
        transform: models.getTotalAnalystsNeededLowerBound,
        fields: [],
      },
      tooltip: true, tooltipIndex: 201,
    },
    ...
    xProjectedWaitTime: {
      alias: 'Projected Avg Wait Time, Upper Bound (mins)',
      type: 'Number',
      onlyWithDatapoint: [ 'month' ],
      calculated: {
        idx: 1,
        transform: models.getProjectedWaitTime,
        fields: [],
      },
      tooltip: true, tooltipIndex: 202,
    },

    xProjectedWaitTime: {
      alias: 'Projected Abandonment Rate, Upper Bound (%)',
      type: 'Number',
      onlyWithDatapoint: [ 'month' ],
      calculated: {
        idx: 1,
        transform: models.getProjectedAbandonmentRate,
        fields: [],
      },
      tooltip: true, tooltipIndex: 203,
    },

  };

  const callMetadataAllDataset = {
    ...callMetadata.dataset,
    name: 'CallsWithTicketsAll',
  };

  const callMetadataAll = { ...callMetadata, dataset: callMetadataAllDataset };
  ...
  const custToMeta = {
    ...
    dtmoOrig: callMetadata,
    dtmo: callMetadataAll,
  ...
  // const customer = cust  || 'master';
  const customer = process.env.CUSTOMER  || 'dtmo';
  ...
  // Return the subtitle based on the date range, if it's available in the data;
  // otherwise, use the subtitle in metadata
  //
  const getDatasetSubtitleNew = data => {
    const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
    const rec = Array.isArray(data) && data.length > 0 ? data[0] : {};
    const range = getDatasetRange(rec);
    if (!range || !range.start || !range.end) {
      return getDatasetSubtitle();
    }
    const start = new Date(range.start);
    const end = new Date(range.end);

    const startMonth = start.getMonth();
    const sm = startMonth < months.length ? months[startMonth] : '';

    const endMonth = end.getMonth();
    const em = endMonth < months.length ? months[endMonth] : '';
    return `${sm} ${start.getFullYear()} - ${em} ${end.getFullYear()}`;
  }

  // Given drawing data representing time values, return the month time range.
  // Return is of the form:
  //   [ 'MM/YYYY', 'MM/YYYY', ... ]
  // Returns empty array if range not found
  //
  // Assumes the 'dataRange' variables are set as constant categorical values(!)
  // in the dataset.  Kinda ugly.
  //
  const getDatasetMonthsNew = data => {
    if (!Array.isArray(data) || data.length === 0) {
      return [];
    }
    return getDatasetMonthsFromRec(data[0]);
  }

  // Given a single record of data, return the dataset range of months
  //
  const getDatasetMonthsFromRec = rec => {
    const range = getDatasetRange(rec);

    if (range.start && range.end) {
      const monthStart = getMonthYear(range.start);
      const monthEnd = getMonthYear(range.end);
      return getMonthList(monthStart, monthEnd);
    } else {
      return [];
    }
  }

  // Return the date range in the dataset, as defined by columns typed 'dataRange'
  // with constant values for the start and end of the range.
  //
  const getDatasetRange = rec => {
    const rangeColumns = getColumnsWithAttr('dataRange');

    return rangeColumns.reduce((i, j) => {
      if (hasValue(j, 'dataRange', 'begin')) {
        return {...i, start: getPivotedValue(j, rec)};
      } else if (hasValue(j, 'dataRange', 'end')) {
        return {...i, end: getPivotedValue(j, rec)};
      }
    }, {start: null, end: null});
  }
  ...
  // Return the list of months between 'monthStart', 'monthEnd' inclusive.
  // Each month is formatted 'm(m)/yyyy'.
  //
  const getMonthList = (monthStart, monthEnd) => {
    const startComps = monthStart.split('/');
    const endComps = monthEnd.split('/');
    if (startComps.length !== 2 || endComps.length !== 2) {
      return [];
    }
    const startMonth = parseInt(startComps[0]);
    const endMonth = parseInt(endComps[0]);
    const startYear = parseInt(startComps[1]);
    const endYear = parseInt(endComps[1]);
    const fullStart = startMonth + startYear * 12;
    const fullEnd = endMonth + endYear * 12;

    const res = [];
    for (let fullMonth = fullStart; fullMonth <= fullEnd; ++fullMonth) {
      const year = Math.floor(fullMonth / 12);
      const month = fullMonth % 12;
      const monthFmt = `${month}/${year}`;
      res.push(monthFmt);
    }
    return res;
  }

  // Return the value embedded in the column name, or null if the value couldn't be found
  //
  const getPivotedValue = (colName, rec) => {
    const colPrefix = `${colName}:`;
    const cols = Object.keys(rec).filter(i => i.startsWith(colPrefix));
    if (cols.length === 1) {
      const col = cols[0];
      return col.substring(colPrefix.length);
    } else {
      return null;
    }
  }
  ...
  // Return a vector of { column, attr-value } for the given attr.
  //
  const getColumnsWithAttrVector = attr => {
    return Object.keys(thisMetadata).filter(i => isTrue(i, attr)).map(i => {
      return { col: i, value: getAttrValue(i, attr, null) };
    });
  }

  // Return true if 'attr' is present
  //
  const hasAttribute = function(col, attr){
  ...
  const getDates = function(){
    return getColsWithType('Date');
  }

  const getDatesAndDateStrings = function(){
    return getColsWithType('Date').concat(getColsWithType('DateString'));
  }

  // Given a date string column, return its output column, or null if there is none
  //
  const getDateOutputCol = dateStringCol => {
    const outputCol = getAttrValue(dateStringCol, 'output');
    return outputCol === '' ? null : outputCol;
  }
  ...
  const getNumericsNotCalculatedOrSingleton = function() {
    return thisMetadata
      ? Object.keys(thisMetadata)
        .map(i => { return {column: i, ...thisMetadata[i]}; })
        .filter(i => i.type==="Number" && !i.calculated && !i.singleton)
        .map(i => i.column)
      : [];
  }

  const getSingletonNumerics = function() {
    return thisMetadata
      ? Object.keys(thisMetadata)
        .map(i => { return {column: i, ...thisMetadata[i]}; })
        .filter(i => i.type==="Number" && i.hasOwnProperty('singleton'))
        .map(i => i.column)
      : [];
  }
  ...
  // Return numeric columns, but filter out columns that are
  // singletons, or that have 'isAverage' set to true, or are calculated fields.
  //
  // We can only average columns that represent sums.
  //
  // Used in dataread withAverages().
  //
  const getAverageableNumerics = function(){
    return thisMetadata
      ? Object.keys(thisMetadata)
        .map(i => { return {column: i, ...thisMetadata[i]}; })
        .filter(i => i.type==="Number" && 
                     !i.isAverage && 
                     !i.hasOwnProperty('singleton') &&
                     !i.calculated)
        .map(i => i.column)
      : [];
  }
  ...
  // Like the above, but only aggregateable categoricals are returned
  //
  function getAggregateCategoricals(){
    return getCategoricals().filter(i => !isTrue(i, 'noAggregate'));
  }
  ...
  // Return true if 'col' is allowed to be calculated
  // for the given 'datapoint'.  It's allowed if its 'onlyWithDatapoint'
  // and its singleton state allow it to be used with the datapoint.
  //
  function isAllowedForDatapoint(datapoint, col) {
    const datapointVec = getAttrValue(col, 'onlyWithDatapoint');

    const withDatapoint = !Array.isArray(datapointVec) || datapointVec.reduce((i, j) => {
      return i || datapoint === j;
    }, false);


    const singleton = getAttrValue(col, 'singleton');
    return withDatapoint && (!singleton || singleton === '' || singleton === datapoint);
  }

  // Given a control, e.g. 'xAxis', return its default column.
  // Return null if none was found.
  //
  function getDefaultColForControl(control) {
    const map = {
      xAxis: 'defaultXValue',
      yAxis: 'defaultYValue',
    ...
    const attr = map.hasOwnProperty(control) ? map[control] : null;
    if (!attr) {
      return null;
    }
    const cols = getColumnsWithAttr(attr);
    if (!Array.isArray(cols) || cols.length === 0) {
      return null;
    }
    const col = cols[0];
    const value = getAttrValue(col, attr);
    if (value !== 'self') {
      return null;
    }
    return col;
  ...
  // Return every non-calculated column name.
  // Like getAll() above.
  //
  const getAllNonCalculated = () => {
    return Object.keys(thisMetadata).filter(i => {
      const colObj = thisMetadata[i];
      return i !== 'dataset' && !colObj.calculated && !colObj.noAggregate;
    });
  }

  // Same as above, but does not depend on the current metadata context.
  //
  const getAllColsForDataset = dataset => {
    const metadata = getMetadata(dataset);
    return metadata
      ? Object.keys(metadata).filter(i => i !== 'dataset').map(i => ({ name: i, ...metadata[i] }))
      : [];
  }

  // Return list of markers, given current datapoint.
  // Return [] if there are none
  //
  const getMarkers = datapointCol => {
    const markerObj = getDatasetAttr('markers');
    if (markerObj && markerObj.datapoint === datapointCol) {
      return Array.isArray(markerObj.data) ? markerObj.data : [];
    } else {
      return [];
    }
  }
  ...
  return {
    ...
    getDatasetLabel,
    getDatasetSubtitle,
    getDatasetSubtitleNew,
    getDatasetMonths,
    getDatasetMonthsNew,
    getDatasetMonthsFromRec,
    getDatasetModel,
    getDatasetDisabled,
    getColumnsWithAttr,
    getColumnsWithAttrTrue,
    getColumnsByAttrValue,
    getColumnsWithAttrVector,
    getAttrValue,
    getFormattedValue,
    applyFormat,
    hasValue,
    ...
    hasAttributeValue,
    getDates,
    getDatesAndDateStrings,
    getDateOutputCol,
    getNumerics,
    getNumericsNotCalculated,
    getNumericsNotCalculatedOrSingleton,
    getSingletonNumerics,
    getVectors,
    getAverageableNumerics,
    getColsWithType,
    aliasToColumn,
    getAxisFormat,
    getAlias,
    getDatapointAlias,
    getCategoricals,
    getAggregateCategoricals,
    getStrings,
    getSearchable,
    getFilters,
    isAllowedForDatapoint,
    getDefaultColForControl,
    transformIsoDates,
    formatIsoDates,
    getBinner,
    getBinnerObject,
